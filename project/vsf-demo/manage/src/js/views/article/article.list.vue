<template>
    <div class="container">
            ...
            <el-table :data="articleList" stripe border height="480" v-loading="loading.articleList">
                ...
                <el-table-column prop="disabled" label="状态" :formatter="disabledStr" width="80">
                    <template slot-scope="scope">
                        <el-tag :type="scope.row.disabled === 1 ? 'primary' : 'info'" close-transition>{{scope.row.disabled | disabledStr}}</el-tag>
                    </template>
                </el-table-column>
                <el-table-column fixed="right" label="操作" width="210">
                    <template slot-scope="scope">
                        <el-button size="small" icon="el-icon-edit" @click="handlerEdit(scope.row)"></el-button>
                        <el-button v-if="scope.row.disabled == 0" size="small" type="info" icon="el-icon-circle-check-outline" @click="handlerPublish(scope.row)">发布</el-button>
                        <el-button v-if="scope.row.disabled == 1" size="small" type="primary" icon="el-icon-circle-close-outline" @click="handlerDisabled(scope.row)">下线</el-button>
                        <el-button size="small" type="danger" icon="el-icon-delete" @click="handlerDelete(scope.row)"></el-button>
                    </template>
                </el-table-column>
            </el-table>
        </div>
        <div class="table-box" style="float: right">
            <el-pagination @size-change="handlerSizeChange" @current-change="handlerAgination" :current-page="formInline.currPage" :page-sizes="[20, 50, 100, 200]" :page-size="formInline.currPage" layout="total, sizes, prev, pager, next, jumper" :total="formInline.total"></el-pagination>
        </div>
        <el-dialog :title="typeTitle" :visible.sync="typeListVisible" width="440px" :before-close="handlerCloseTypeList">
            <div>
                <el-table :data="articleTypeList" stripe border v-loading="loading.articleTypeList">
                    ...
                    <el-table-column fixed="right" align="center" label="操作" width="160">
                        <template slot-scope="scope">
                            <el-button size="small" icon="el-icon-edit" @click="toUpdateAType(scope.row)"></el-button>
                            <el-button size="small" type="danger" icon="el-icon-delete" @click="handlerDeleteAType(scope.row)"></el-button>
                        </template>
                    </el-table-column>
                </el-table>
        ...
        <el-dialog :title="typeTitle" :visible.sync="typeInfoVisible" width="360px" :before-close="handlerCloseTypeInfo">
...
export default {
    ...
    methods: {
        loadArticleList() {
            this.$.post(HttpUrl.findArticleAll, this.formInline).then(
                results => {
        ...
        loadArticleTypeList() {
            return this.$.get(HttpUrl.findArticleTypeAll).then(
                articleTypeList => {
        ...
        handlerPublish(item) {
            this.$confirm("发布文章", "提示", {
                ...
                .then(() => {
                    this.$.get(`${HttpUrl.publishArticle}?id=${item.id}`).then(
                        results => {
                            this.$message({
                                type: "success",
                                message: "操作成功",
        ...
        saveArticleType() {
            this.$.post(HttpUrl.saveArticleType, this.articleType).then(
                results => {
        ...
        handlerEdit(item) {
            this.$router.push(`/article/articlePulish/${item.id}`);
        },
        ...
        handlerCloseTypeList(done) {
            done();
        },
        handlerCloseTypeInfo(done) {
            done();
        },
        typeStr({ type }) {
