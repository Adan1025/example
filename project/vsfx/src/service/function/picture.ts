...
export class PictureService extends BaseService {
    ...
    async getAll({ name = '', noPage = false, vague = false, pageSize = 20, currPage = 1, used = 'all' }: AllParam) {
        ...
        let query = this.getRepository(Picture).createQueryBuilder("picture")
            .select([
                'picture.id id',
                'picture.alt alt',
                'picture.name name',
            ...
            countQuery = this.getRepository(Picture).createQueryBuilder("picture")
