...
export class UsersMenuService extends BaseService implements UsersMenuInterface {
    ...
    async getUsersMenuList({ parentId = null, menuName = null, disabled = null, isMenu = null }: UsersMenuOption) {
        let query = this.getRepository(UsersMenu).createQueryBuilder("UsersMenu").where('1=1');
        if (isInteger(isMenu)) {
    ...
    async getUsersMenuListByParentId(parentId) {
        return await this.getRepository(UsersMenu).createQueryBuilder("UsersMenu")
            .select(['id', 'menuName'])
            .where('parentId=:parentId', { parentId }).orderBy('parentId').orderBy('sort', 'ASC').getRawMany();
    }
    /**
     * 菜单添加
     * @param {*} params 
