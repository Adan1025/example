...
export class ArticleService extends BaseService implements ArticleInterface {
    ...
    async findAllArticle({ usersId = 0, articleTypeId = 0, type = 0, identity = '', nickName = '', desabled = null, pageSize = 20, currPage = 1 }: findAllArticleD) {
        ...
        let query = this.getRepository(Article).createQueryBuilder("article")
            ...
            countQuery = this.getRepository(Article).createQueryBuilder("article")
                .leftJoinAndSelect('article.users', 'users')
                .leftJoinAndSelect('article.articleType', 'articleType')
        ...
        if (identity) {
            ...
            countQuery = countQuery.andWhere('users.identity=:identity', { identity });
    ...
    async findHotArticle(type, identity) {
        ...
        let articleList: Array<Article> = await query.orderBy('article.visitors', 'DESC').offset(0).limit(5).printSql().getRawMany();
        return articleList;
    }
